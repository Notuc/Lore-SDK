// ─────────────────────────────────────────────────────────────
// complete.routes.ts
// POST /complete — waits for the full AI response.
// Used when the game needs the complete reply before
// doing anything (e.g. generating a quest description).
// ─────────────────────────────────────────────────────────────
/**
...
function handleError(err: unknown, reply: FastifyReply): FastifyReply {
  if (err instanceof LoreError) {
    ...
    return (reply as { status: (n: number) => { send: (b: unknown) => unknown } })
      .status(status)
  ...
  return (reply as { status: (n: number) => { send: (b: unknown) => unknown } })
    .status(500)
    .send({ error: "UNKNOWN", message: String(err) });
}
*/ 
